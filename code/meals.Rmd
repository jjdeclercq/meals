...
made_menu <- make_menu(recipes_expand, ingredients, meals)
...
recipes_expand %>% filter(recipe %in% meals$Meal[c(5,7,8)]) %>% select(-variant) %>% pivot_longer(., - recipe) %>% filter(value ==1) %>%
  group_by(recipe, name) %>%tally()
```
...
MEAL <- MEAL %>% mutate(sub4 = sum(c_across(all_of(reaching))), can.make4 = 1*(sub4 == n.ingredients),
