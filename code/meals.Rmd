...
To do as of July 2022
  ...
  - Filter food based on class? All foods that contain noodles, for example. Or beef, pork?
  - Tags: Favorite, chili, stew
...
# which(made_menu$Selected==1)
  # new.ingredients %<>% mutate(across(c(sometimes, equal), ~ifelse(.x=="", NA, .x)))
```
...
```{r}
concerts <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1F1bi8Y12cnmyCnNB3HWpVdj9oi918ohHi7qBITB73Ks/edit#gid=146938949", sheet = "Concerts") 

concerts %>%
  group_by(Date) %>% tally() %>% arrange(desc(n))

concerts %>%
  group_by(Band) %>% tally() %>% arrange(desc(n))
```
